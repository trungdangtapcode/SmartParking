...
def annotate(img_pil, id_label, attributes, tx, ty, bx, by, color, font, plate=None):
    ...
    # Add plate on top of ID if available
    if plate:
        text = [f"Plate: {plate}", id_label] + [f"{k}: {get_attribute_value(k, v)}" for k, v in attributes.items()]
    else:
...
class Video:
    ...
    def render_tracks(self, frame, track_ids, track_bboxes, attributes, plates=None):
        overlay = Image.fromarray(
            np.zeros((frame.shape[0], frame.shape[1], 4), dtype=np.uint8), "RGBA")
        if plates is None:
            plates = [None] * len(track_ids)
        for track_id, bbox, attrib, plate in zip(track_ids, track_bboxes, attributes, plates):
            tx, ty, w, h = bbox
            ...
            overlay = annotate(overlay, str(track_id), attrib,
                               tx, ty, bx, by, color, self.font, plate=plate)
...
class DisplayVideo(Video):
    ...
    def update(self, frame, track_ids, bboxes, attributes, plates=None):
        frame = self.render_tracks(frame, track_ids, bboxes, attributes, plates)
    ...
    for frame_idx, frame in enumerate(video_in):
        ...
        for track_idx, ptr in active_tracks.items():
            ...
            if track.frames[ptr] == frame_idx:
                ...
                plates.append(track.plate if hasattr(track, 'plate') else None)
        ...
        video_out.update(frame, track_ids, bboxes, attribs, plates)
