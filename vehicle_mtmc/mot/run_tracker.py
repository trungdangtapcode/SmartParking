...
def run_mot(cfg: CfgNode):
    ...
    if cfg.MOT.REID_TRT and os.path.exists(cfg.MOT.REID_TRT):
        # Use TensorRT for maximum speed (5-10x faster than PyTorch)
        ...
        extractor = TensorRTFeatureExtractor(cfg.MOT.REID_TRT)
    elif cfg.MOT.REID_ONNX and os.path.exists(cfg.MOT.REID_ONNX):
        # Use ONNX Runtime for faster inference (2-3x faster than PyTorch)
        log.info(f"Loading ONNX ReID model from {cfg.MOT.REID_ONNX}")
        from reid.onnx_feature_extractor import ONNXFeatureExtractor
        extractor = ONNXFeatureExtractor(cfg.MOT.REID_ONNX, use_gpu=device.type == 'cuda')
    else:
        # Use PyTorch model
    ...
    if cfg.MOT.TRACKER == "deepsort":
        ...
        MIN_CONFID = 0.05 # changed from 0.2
    else:
