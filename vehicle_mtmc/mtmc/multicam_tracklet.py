...
class MulticamTracklet:
    ...
    def finalize(self):
        ...
        # Find first non-null plate from any camera
        master_plate = None
        for track in self._tracks:
            if track.plate is not None:
                master_plate = track.plate
                break
        
        # Propagate plate and ID to all tracks
        for track in self._tracks:
            track.track_id = self.id
            if master_plate is not None:
                track.plate = master_plate
