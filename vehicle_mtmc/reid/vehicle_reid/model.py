        ...
        if model_subtype in ("50", "default"):
            if ibn:
                model_ft = torch.hub.load(
                    'XingangPan/IBN-Net', 'resnet50_ibn_a', pretrained=True, trust_repo=True)
            else:
...
# Define the MobileNetV4-based Model
class ft_net_mobilenet(nn.Module):

    def __init__(self, class_num, droprate=0.5, circle=False, linear_num=512, model_subtype="small"):
        super().__init__()
        
        if model_subtype == "default":
            model_subtype = "small"
        
        # MobileNetV4 feature dimensions by variant (from timm model configs)
        feature_dims = {
            "small_035": 336,          # mobilenetv4_conv_small_035 (no pretrained weights)
            "small_050": 480,          # mobilenetv4_conv_small_050 - SMALLEST with weights
            "small": 960,              # mobilenetv4_conv_small
            "medium": 960,             # mobilenetv4_conv_medium
            "large": 960,              # mobilenetv4_conv_large
            "hybrid_medium": 1280,     # mobilenetv4_hybrid_medium
            "hybrid_medium_075": 1280, # mobilenetv4_hybrid_medium_075
            "hybrid_large": 1280,      # mobilenetv4_hybrid_large
            "hybrid_large_075": 1280,  # mobilenetv4_hybrid_large_075
        }
        
        # Model name mapping
        model_names = {
            "small_035": "mobilenetv4_conv_small_035",
            "small_050": "mobilenetv4_conv_small_050",
            ...
            "hybrid_large": "mobilenetv4_hybrid_large",
            "hybrid_large_075": "mobilenetv4_hybrid_large_075",
        }
        
        feature_dim = feature_dims.get(model_subtype, 960)
        model_name = model_names.get(model_subtype, "mobilenetv4_conv_small_035")
        
        # Some variants don't have pretrained weights
        no_pretrained = ["small_035", "hybrid_medium_075", "hybrid_large_075"]
        use_pretrained = model_subtype not in no_pretrained
        
        # Create MobileNetV4 using timm
        model_ft = timm.create_model(model_name, pretrained=use_pretrained, num_classes=0)  # num_classes=0 removes classifier
        ...
        self.classifier = ClassBlock(
            feature_dim, class_num, droprate, linear=linear_num, return_f=circle)

    def forward(self, x):
        # For MobileNetV4, use full forward pass (includes norm_head for hybrid models)
        # timm model was created with num_classes=0, so it returns features directly
        x = self.model(x)  # Shape: (batch, feature_dim)
        x = self.classifier(x)
        return x
