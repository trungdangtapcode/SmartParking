...
from model import ft_net, ft_net_dense, ft_net_hr, ft_net_swin, ft_net_efficient, ft_net_NAS, ft_net_mobilenet, PCB
...
def create_model(n_classes, kind="resnet", **kwargs):
    ...
    if kind == "resnet":
        ...
        return ft_net_mobilenet(n_classes, **kwargs)
    elif kind == "NAS":
...
def load_model_from_opts(opts_file, ckpt=None, return_feature=False, remove_classifier=False):
    ...
    if model_type in ("resnet", "resnet_ibn"):
        ...
        model = create_model(n_classes, "mobilenet", droprate=droprate,
                             circle=return_feature, linear_num=linear_num, model_subtype=model_subtype)
    elif model_type == "NAS":
