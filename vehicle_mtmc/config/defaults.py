...
# ONNX model for faster ReID inference (optional)
# If set, will use ONNX Runtime instead of PyTorch (2-3x faster)
C.MOT.REID_ONNX = None

# TensorRT engine for maximum ReID speed (optional)
# If set, will use TensorRT instead of ONNX/PyTorch (5-10x faster)
...
# enable license plate recognition (ALPR)
C.MOT.ENABLE_ALPR = False
...
# Target output fps for the annotated stream (per camera)
C.LIVE.TARGET_FPS = 15

# HTTP port for MJPEG streaming (one endpoint per camera)
C.LIVE.MJPEG_PORT = 8080

# Cosine similarity threshold to merge tracks across cameras online
C.LIVE.SIMILARITY_THRESHOLD = 0.5

# Exponential moving average factor for global feature updates
C.LIVE.GLOBAL_FEATURE_EMA = 0.6

# Drop a global track if it has not been updated within this many seconds
C.LIVE.MAX_INACTIVE_SECS = 5.0

# Maximum tick lead (in ticks) a fast camera can have over the slowest one
C.LIVE.MAX_SKEW_TICKS = 1

# Seconds before a stalled camera is removed from synchronization (prevents whole system blocking)
C.LIVE.STALL_SECONDS = 5.0

# Minimum frames in a track before it can enter live clustering
C.LIVE.MIN_TRACK_FRAMES = 3

# Seconds between live clustering runs (to accumulate a short window of evidence)
C.LIVE.CLUSTER_INTERVAL = 0.5

# Loop video inputs when they end (useful for demo clips)
C.LIVE.LOOP_VIDEO = True

# Sleep duration when a frame is not yet available (seconds)
C.LIVE.SLEEP_ON_EMPTY = 0.02
