...
def expand_relative_paths(root_cfg: CN):
    ...
    for it in c.EXPRESS.CAMERAS:
        for k, v in list(it.items()):
            # Skip non-path fields
            if k in ["valid_zonepaths", "tracked_classes", "enable_alpr", "name"]:
                continue
            # Handle detector field (only expand if it's a path, not a model name)
            if k == "detector" and v is not None:
                if v.endswith('.pt') or '/' in v:
