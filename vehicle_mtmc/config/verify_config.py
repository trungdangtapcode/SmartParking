...
common_mot_checks = {
    ...
    "REID_MODEL_CKPT": lambda x: x is None or os.path.isfile(x),
    "REID_FP16": _is(bool),
    "REID_BATCHSIZE": lambda x: _is(int)(x) and x >= 1,
    "DETECTOR": lambda x: x.startswith("yolov5") or x.startswith("yolov8") or x.startswith("yolo11") or x.endswith(".pt") or os.path.exists(x) or os.path.exists(f"{x}.pt"),
    "TRACKER": lambda x: x in ["deepsort", "bytetrack_iou"],
    "TRACKED_CLASSES": lambda x: _is(list)(x) and all(_is(int)(y) for y in x),
    "SHOW": _is(bool),
...
def _check_express_camera(d: dict):
    if "video" not in d:
        return False
    all_keys = ["video", "detection_mask", "zone_mask_dir", "valid_zonepaths", "calibration", "detector", "tracked_classes", "enable_alpr"]
    for k, v in d.items():
        if k not in all_keys:
            return False
        # Skip validation for enable_alpr (boolean)
        if k == "enable_alpr":
            continue
        # Use common_mot_checks for detector and tracked_classes
        if k.upper() in common_mot_checks:
            check_fn = common_mot_checks[k.upper()]
        else:
